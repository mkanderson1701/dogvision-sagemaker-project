...
def create_data_loader(args):
    ...
    train_loader = DataLoader(train_data, batch_size=args.batch_size,
            pin_memory=pin_mem, shuffle=True, num_workers = 4)
...
def save_model(model, optimizer, args):
    filepath = os.path.join(args.model_dir, 'model.pt')
...
def predict_fn(input_data, model):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(f'device is {device}, moving input_data')
    input_data = input_data.to(device)
    model.to(device)
    logger.info('set eval mode again just in case')
    model.eval()
    logger.info('final setup, return logits')
    return model(input_data)


def model_fn(model_dir):
    logger.info('running model_fn...')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logger.info(f'device is {device}...')
    logger.info('setting up resnet...')
    model = resnet152()
    logger.info(f'resnet configured: {model}...')
    model.fc = MyClassifier(256, 0)
    logger.info(f'classifier configured: {model}...')
    with open(os.path.join(model_dir, 'model.pt'), "rb") as f:
        logger.info(f'checkpoint file opened, loading state dict...')
...
def main(args):
    ...
    train(model, train_loader, valid_loader, criterion, optimizer, args, hook)
